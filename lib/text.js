...
export function cleanSummary(summary) {
  return String(summary)
    ...
    .replace(/^\s*(?:[가-힣A-Za-z]{2,8}\s+){0,3}[가-힣A-Za-z]{2,8}\s*(?:기자|특파원|통신원|객원기자)\s*=\s*/, '')
    .replace(/\s*<[^>]*>\s*/g, ' ')
    .replace(/\s*\.{3,}\s*$/, '…')
    .replace(/\s+/g, ' ')
    .trim();
...
// ── 신문체 → 존댓말 ──────────────────────────────────────────
// "정부는 12억원으로 유지했다." → "정부는 12억원으로 유지했습니다."
//
// 통신사 리드는 전부 '~했다' 체라 아침에 읽기엔 딱딱하다. 문장의 **끝**에 있는
// 종결어미만 바꾼다. 문장 중간의 "~했다고 밝혔다"의 앞부분은 건드리지 않는다.
...
function jongseong(ch) {
  ...
  if (code < 0 || code > 11171) return -1; // 한글 음절이 아님
  return code % 28;
}

function withJongseong(ch, jong) {
  const code = ch.codePointAt(0) - HANGUL_BASE;
  return String.fromCodePoint(HANGUL_BASE + code - (code % 28) + jong);
}

/**
 * 문장 끝의 '~다'를 '~습니다/~ㅂ니다'로 바꾼다.
 *   했다 → 했습니다   (받침 ㅆ)
 *   한다 → 합니다     (받침 ㄴ → ㅂ)
 *   이다 → 입니다     (받침 없음 → ㅂ)
 *   없다 → 없습니다   (그 밖의 받침)
 */
function politeEnding(word) {
  if (!word.endsWith('다') || word.length < 2) return word;
  const stem = word.slice(0, -1);
  const last = stem[stem.length - 1];
  const jong = jongseong(last);
  if (jong < 0) return word;

  if (jong === JONG_NONE || jong === JONG_N || jong === JONG_L) {
    return stem.slice(0, -1) + withJongseong(last, JONG_B) + '니다';
  }
  return stem + '습니다';
}

/** 문장(들)의 종결어미만 존댓말로 바꾼다. */
export function toPolite(text) {
  return String(text).replace(/([가-힣]+다)(?=\s*[.!?]|\s*$)/g, (m) => politeEnding(m));
}
