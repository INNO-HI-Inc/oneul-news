...
import { truncate, splitSentences } from './text.js';
...
function fallbackItem(cluster) {
  ...
  const sentences = splitSentences(lead).slice(0, 2).join(' ');
  return {
    id: cluster.id,
    headline: truncate(cluster.title, 40),
    summary: truncate(sentences || cluster.title, 180),
    why: '',
  };
}

function fallbackGreeting(clusters, dateLabel) {
  const top = clusters[0];
  if (!top) return `${dateLabel} 아침입니다. 오늘도 좋은 하루 보내세요.`;
  return `${dateLabel} 아침입니다. 밤사이 «${truncate(top.title, 28)}» 소식이 가장 많이 오르내렸어요.`;
}

/**
 * @returns {{ greeting: string, byId: Map<string, object>, engine: 'claude'|'lead' }}
 */
export async function summarize(clusters, { dateLabel, log = () => {} } = {}) {
  const hasKey = Boolean(process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_AUTH_TOKEN);

  if (!hasKey || !clusters.length) {
    if (!hasKey) log('  · ANTHROPIC_API_KEY 없음 → 기사 리드 문장으로 요약합니다.');
    return {
      greeting: fallbackGreeting(clusters, dateLabel),
